...
isHost(isHost)
{
	if(isHost)
	{
		...
		button->setFunc([this]()
		{
			...
			{
				sf::Packet packet;
				packet << PacketType::START << id++ << seed;
				
				while(client->send(packet) != sf::Socket::Done) {}
			}
			
			currentState.state = new StateGame(playerCount, seed, 0, nullptr, std::move(clients));
		...
		if(server->receive(packet) == sf::Socket::Done)
		{
			...
			switch(packetType)
			{
				...
				case START:
				{
					...
					packet >> id >> seed;
					currentState.state = new StateGame(playerCount, id, seed, server, std::move(clients));
					currentState.destroyLast = true;
					server = nullptr;
					break;
				}
